...
bool isConfigPin(uint8_t pin) {
  for (size_t i = 0; i < CONFIG_COUNT; ++i) {
    if (CONFIG_PINS[i] == pin) return true;
  }
  return false;
}
...
uint8_t loadPinMode(uint8_t pin) {
  ...
  if (mode > 2 || (mode == 2 && !isAdcCapable(pin))) mode = 0;
  return mode;
}
...
String jsonStatus() {
  ...
  j += "{";
  j += "\"ip\":\"";
  j += (WiFi.status() == WL_CONNECTED ? WiFi.localIP().toString() : WiFi.softAPIP().toString());
  j += "\",";
  j += "\"mode\":\"" + String(WiFi.status() == WL_CONNECTED ? "AP+STA" : "AP") + "\",";
  j += "\"pins\":[";
...
void handleConfig() {
  ...
  uint8_t mode;
  if (requested == "out") mode = 1;
  else if (requested == "adc") mode = 2;
  else if (requested == "in") mode = 0;
  else {
...
void handleWiFiStatus() {
  ...
  json += ",\"ssid\":\"";
  json += WiFi.status() == WL_CONNECTED ? WiFi.SSID() : wifiSSID;
  json += "\"";
  json += ",\"localIP\":\"";
  ...
  json += "\"";
  json += ",\"dhcp\":";
  json += (!wifiStatic ? "true" : "false");
  json += ",\"staticIP\":\"";
  json += wifiIP.toString();
  json += "\"";
  json += ",\"gateway\":\"";
  json += wifiGateway.toString();
  json += "\"";
  json += ",\"subnet\":\"";
  json += wifiSubnet.toString();
  json += "\"";
  json += ",\"dns\":\"";
  json += wifiDNS.toString();
  json += "\"";
  json += "}";
...
void setup() {
  ...
  server.on("/", HTTP_GET, []() { if (!requireAuth()) return; handleRoot(); });
  server.on("/index.html", HTTP_GET, []() { if (!requireAuth()) return; handleRoot(); });
  ...
  server.on("/api/status", HTTP_GET, []() { if (!requireAuth()) return; handleStatus(); });
  server.on("/api/set", HTTP_GET, []() { if (!requireAuth()) return; handleSet(); });
  server.on("/api/config", HTTP_GET, []() { if (!requireAuth()) return; handleConfig(); });
  server.on("/api/wifi/status", HTTP_GET, []() { if (!requireAuth()) return; handleWiFiStatus(); });
  server.on("/api/wifi/scan", HTTP_GET, []() { if (!requireAuth()) return; handleWiFiScan(); });
  server.on("/api/wifi/save", HTTP_GET, []() { if (!requireAuth()) return; handleWiFiSave(); });
